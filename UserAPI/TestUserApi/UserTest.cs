...
namespace TestUserApi
{
    public class UnitTest1
    {
        ...
        public void TestConnection()
        {
            ...
            var check = client.GetAsync(string.Format("http://dlsuserapi.azurewebsites.net/api/users/{0}", 4)).Result;
        ...
        public void TestCRDUser()
        {
            client = new HttpClient();
            // count
            var url = "http://dlsuserapi.azurewebsites.net/api/users/";

            var obj = new ExpandoObject();
            obj.TryAdd("isAdmin", false);
        ...
        public void TestCRDMultipleUsers()
        {
            ...
            // Read
            var userjson = client.GetStringAsync(url + "/basdamgaard/1234567890").Result;
            var newObject1 = JsonConvert.DeserializeObject<ExpandoObject>(userjson);

            userjson = client.GetStringAsync(url + "/emilesp/1234567890").Result;
            var newObject2 = JsonConvert.DeserializeObject<ExpandoObject>(userjson);

            userjson = client.GetStringAsync(url + "/dánjalDJ/1234567890").Result;
            var newObject3 = JsonConvert.DeserializeObject<ExpandoObject>(userjson);

            // Delete first
            // Delete
            var contentdel1 = client.DeleteAsync(url + '/' + newObject1.ElementAt(2).Value).Result;
            var contentdel2 = client.DeleteAsync(url + '/' + newObject2.ElementAt(2).Value).Result;
            var contentdel3 = client.DeleteAsync(url + '/' + newObject3.ElementAt(2).Value).Result;
            ...
            Assert.True(string.Equals(obj1.ElementAt(1).Value, newObject1.ElementAt(1).Value), "First object could not be found after being inserted");
            Assert.True(string.Equals(obj2.ElementAt(1).Value, newObject2.ElementAt(1).Value), "Second object could not be found after being inserted");
            Assert.True(string.Equals(obj3.ElementAt(1).Value, newObject3.ElementAt(1).Value), "Third object could not be found after being inserted");
            Assert.True(contentdel1.IsSuccessStatusCode, "Delete first user failed");
            Assert.True(contentdel2.IsSuccessStatusCode, "Delete second user failed");
            Assert.True(contentdel3.IsSuccessStatusCode, "Delete third user failed");
        }
        ...
        {
            client = new HttpClient();
            var url = "http://dlsuserapi.azurewebsites.net/api/users/-9999";
        ...
        {
            client = new HttpClient();
            var url = "http://dlsuserapi.azurewebsites.net/api/users/hello";
        ...
        {
            client = new HttpClient();
            var url = "http://dlsuserapi.azurewebsites.net/api/users/5";
        ...
        [Fact]
        public void TestCreateNotExistingProperties()
        {
            client = new HttpClient();
            var url = "http://dlsuserapi.azurewebsites.net/api/users";
        ...
        [Fact]
        public void TestCreateWithWrongDataType()
        {
            client = new HttpClient();
            var url = "http://dlsuserapi.azurewebsites.net/api/users";
        ...
        [Fact]
        public void TestCantCreateAdmin()
        {
            client = new HttpClient();
            var url = "http://dlsuserapi.azurewebsites.net/api/users";
            ...
            Assert.False(content.IsSuccessStatusCode, "Fail: Created admin account");
        }

        [Fact]
        public void TestCantUpdateToAdmin()
        {
            client = new HttpClient();
            var url = "http://dlsuserapi.azurewebsites.net/api/users";
            ...
            var obj2 = new ExpandoObject();
            obj2.TryAdd(newObject.ElementAt(0).Key, true);
            obj2.TryAdd(newObject.ElementAt(1).Key, newObject.ElementAt(1).Value);
            obj2.TryAdd(newObject.ElementAt(3).Key, newObject.ElementAt(3).Value);
            obj2.TryAdd(newObject.ElementAt(4).Key, newObject.ElementAt(4).Value);

            // Update
            jsonObject = JsonConvert.SerializeObject(obj);
            stringcontent = new StringContent(jsonObject);
            stringcontent.Headers.ContentType = MediaTypeHeaderValue.Parse("application/json");
            var contentupdate = client.PutAsync(url+ "/" + newObject.ElementAt(2).Value, stringcontent).Result;

            Assert.True(content.IsSuccessStatusCode, "Post user failed");
            Assert.True(string.Equals(newObject.ElementAt(1).Value, obj.ElementAt(1).Value), "Read wrong user");
            Assert.False(contentupdate.IsSuccessStatusCode, "Fail: Update to admin worked");
        }
    }
}
