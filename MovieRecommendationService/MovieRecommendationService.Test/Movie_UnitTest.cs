    ...
    public class Movie_UnitTest
    {
        ...
        public void Can_Update_Movie()
        {
            try
            {
                //Arrange
                Movie movie = GenerateMovie();
                movie.GenreList = new List<Genre>() { new Genre() { GenreName = "start1" }, new Genre() { GenreName = "start2" } };
                handler.CreateMovie(movie);
                ...
                Movie newMovie = new Movie()
                {
                    ...
                    Title = "Can_Update_Movie Test",
                    ReleaseYear = movie.ReleaseYear +1
                };
                newMovie.GenreList = new List<Genre>() { new Genre() { GenreName ="TESTGENRENAME1" }, new Genre() { GenreName = "TESTGENRENAME2" } };

                handler.UpdateMovie(newMovie);

                // Assert
                Movie foundMovie = handler.GetMovieWithGenres(movie.MovieId);

                Assert.AreNotSame(movie, foundMovie, "the found movie was the same object");
                Assert.AreNotSame(newMovie, foundMovie, "the found movie was the same object");
                Assert.IsNotNull(foundMovie);
                Assert.AreEqual(movie.MovieId, foundMovie.MovieId);
                Assert.AreEqual(newMovie.MovieId, foundMovie.MovieId);
                Assert.AreEqual(newMovie.Title, foundMovie.Title);
                Assert.AreEqual(newMovie.ReleaseYear, foundMovie.ReleaseYear);
            }
            catch (Exception ex)
            {
                Assert.Fail("Threw Exception: " + ex.ToString());
